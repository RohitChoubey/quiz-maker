...
class Quiz_Maker_Admin
{
    ...
    public function add_question_rows(){
        if ((isset($_REQUEST["add_question_rows"]) && wp_verify_nonce($_REQUEST["add_question_rows"], 'add_question_rows')) || 
            (isset($_REQUEST["add_question_rows_top"]) && wp_verify_nonce($_REQUEST["add_question_rows_top"], 'add_question_rows_top'))) {
    ...
    public function ays_quick_start(){
        ...
        foreach ($questions as $question_key => $question) {
            $wpdb->insert($questions_table, array(
                'category_id'   => esc_sql( $questions_cat[$question_key] ),
                'question'      => esc_sql( stripslashes($question) ),
                'published'     => 1,
                'type'          => esc_sql( $questions_type[$question_key] ),
    ...
    */
    ...
    public function ays_quiz_sale_baner(){
        if(isset($_POST['ays_quiz_sale_btn'])){
            update_option('ays_quiz_sale_notification', 1); 
        ...
        if(intval($days_diff) > 0 ){
            update_option('ays_quiz_sale_notification', 0); 
        }

        $ays_quiz_ishmar = intval(get_option('ays_quiz_sale_notification'));
        ...
        if($ishmar == 0 ){
            ...
            $content[] = '<div id="ays-quiz-dicount-month-main" class="notice notice-success is-dismissible ays_quiz_dicount_info">';
                $content[] = '<div id="ays-quiz-dicount-month" class="ays_quiz_dicount_month">';
                    $content[] = '<a href="https://bit.ly/3D5dt0E" target="_blank" class="ays-quiz-sale-banner-link"><img src="' . AYS_QUIZ_ADMIN_URL . '/images/mega_bundle_logo_box.png"></a>';

                    $content[] = '<div class="ays-quiz-dicount-wrap-box">';

                        $content[] = '<strong style="font-weight: bold;">';
                            $content[] = __( "Limited Time <span style='color:#E85011;'>55%</span> SALE on <br><span><a href='https://bit.ly/3D5dt0E' target='_blank' style='color:#E85011; text-decoration: underline;'>Mega Bundle</a></span> (Quiz + Survey + Poll)!", AYS_QUIZ_NAME );
                        $content[] = '</strong>';

                        $content[] = '<br>';

                        $content[] = '<strong>';
                                $content[] = __( "Hurry up! Ends on November 15. <a href='https://bit.ly/3D5dt0E' target='_blank'>Check it out!</a>", AYS_QUIZ_NAME );
                        $content[] = '</strong>';
                    ...
                    $content[] = '<a href="https://bit.ly/3D5dt0E" class="button button-primary ays-button" id="ays-button-top-buy-now" target="_blank" style="height: 32px; display: flex; align-items: center; font-weight: 500; " >' . __( 'Buy Now !', AYS_QUIZ_NAME ) . '</a>';
                $content[] = '</div>';
            $content[] = '</div>';
